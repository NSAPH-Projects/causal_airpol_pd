...
test$method_lab <- factor(test$meth, c("MSM", "GPS matching"),
                       c("MSM", "GPS~Matching"))
test$pol_lab <- factor(test$pol, c("PM2.5", "NO2", "O3"),
                       c("PM[2.5]", "NO[2]", "O[3]"))
test %>%
  ...
  facet_nested(pol_lab ~ method_lab, scales = "free_y", labeller = label_parsed) +
  coord_flip() +
