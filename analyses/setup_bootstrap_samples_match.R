...
dat <- read_fst("~/nsaph_projects/vwang_causal_airpol_pd/data/pd_merged_trim.fst",
                           as.data.table = TRUE) %>% filter(outer_pm_tod==0)
# dat <- read_fst("~/nsaph_projects/vwang_causal_airpol_pd/data/pd_merged_trim.fst",
#                 as.data.table = TRUE) %>% filter(outer_no2_tod==0)
# dat <- read_fst("~/nsaph_projects/vwang_causal_airpol_pd/data/pd_merged_trim.fst",
#                 as.data.table = TRUE) %>% filter(outer_o3_tod==0)

dat[, .N] # 35885192; 35885221; 35885341
...
zips <- dat[, unique(zip)]
length(zips) # n = 34052; 34652; 34274
round(2 * sqrt(length(zips))) # m = 369; 372; 370
...
for (i in 1:B) {
  ...
  write_fst(df_boot, paste0("~/nsaph_projects/vwang_causal_airpol_pd/code/bootstrap_code/boot_dat_match_o3/boot_", i, ".fst"))
}
